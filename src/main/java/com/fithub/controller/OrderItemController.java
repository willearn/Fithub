...
//    }
	...
	@GetMapping("/getOrderItemAmountByClassId")
	public ResponseEntity<?> getOrderItemAmountByClassId(@RequestParam(value = "classId") int classId) {
		try {
			Map<String, Object> orderItemAmount = orderItemService.getOrderItemAmountByClassId(classId);
			if (orderItemAmount != null) {
				return new ResponseEntity<>(orderItemAmount, HttpStatus.OK);
			}
			return new ResponseEntity<>(HttpStatus.NOT_FOUND);
		} catch (Exception e) {
			return new ResponseEntity<>("An error occurred: " + e.getMessage(), HttpStatus.INTERNAL_SERVER_ERROR);
